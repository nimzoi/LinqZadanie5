...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            var result = Emps.Select(e => new { e.Ename, e.Job, e.HireDate }).ToList<object>();
            result.Add(new { Ename = "Brak wartości", Job = (string)null, HireDate = (DateTime?)null });
            return result;
        }
        ...
        {
            return Emps.GroupBy(e => e.Dept)
                       .Where(g => g.Key != null && g.Count() > 1)
                       .Select(g => new { name = g.Key.Dname.ToUpper(), numOfEmployees = g.Count() })
                       .ToList();
        }
